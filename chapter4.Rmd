...
colnames(test)
```
Next we are going to fit a linear discriminant analysis (LDA) model to the training data. we'll use the newly created "crime" variable as the target and all others as the predictors.
```{r}
...
```
To continue testing, we are going to remove the "crime" categorical variable from the test data set. No worries, the crime categories will be saved for possible later use.
