...
```
There appears a statistically significant correlation between absences and high alcohol usage. Interestingly, all of school=MS, family size = less than 3, and age include 1 within the 95 % confidence interval, which implies that the result is not statistically significant. This result rejects my personal hypothesis as the data does not show obvious connections between the chosen variables and high alcohol usage.

## Predictive power of the model
The only variable with a statistically significant connection to high alcohol usage is "absences". I'll explore its prediction capabilities next.
```{r}
# fit the model
